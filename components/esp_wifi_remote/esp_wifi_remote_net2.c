...
{
    if (!h || !buffer) {
        ESP_LOGE(TAG, "esp_wifi_remote_channel_rx: invalid handle or buffer");
...
{
    if (!buffer) {
        ESP_LOGE(TAG, "internal_tx: invalid buffer");
        return -1;
    }
...
{
    if (!buffer || !s_wifi_netifs[WIFI_IF_STA]) {
        ESP_LOGD(TAG, "receive_sta: netif not initialized or buffer is NULL");
...
{
    if (!buffer || !s_wifi_netifs[WIFI_IF_AP]) {
        ESP_LOGD(TAG, "receive_ap: netif not initialized or buffer is NULL");
        return ESP_ERR_INVALID_STATE;
    }
