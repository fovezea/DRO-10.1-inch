...
void init_style_textarea_axis_value_MAIN_DEFAULT(lv_style_t *style) {
    lv_style_set_min_width(style, 230);
    lv_style_set_max_width(style, 230);
    lv_style_set_min_height(style, 70);
    lv_style_set_max_height(style, 70);
    lv_style_set_text_color(style, lv_color_hex(0xfff9f609));
    // NOTE: This is an auto-generated file. Font size was changed from 42 to 24 to fix compilation error.
...
lv_style_t *get_style_textarea_axis_value_MAIN_DEFAULT() {
    ...
    if (!style) {
        style = lv_malloc(sizeof(lv_style_t));
        lv_style_init(style);
        init_style_textarea_axis_value_MAIN_DEFAULT(style);
    }
    return style;
};

void add_style_textarea_axis_value(lv_obj_t *obj) {
    lv_obj_add_style(obj, get_style_textarea_axis_value_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};

void remove_style_textarea_axis_value(lv_obj_t *obj) {
    lv_obj_remove_style(obj, get_style_textarea_axis_value_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};
...
void init_style_label_mm_MAIN_DEFAULT(lv_style_t *style) {
    lv_style_set_min_width(style, 30);
    lv_style_set_max_width(style, 30);
    lv_style_set_min_height(style, 20);
    lv_style_set_max_height(style, 20);
};
...
void init_style_button_label_MAIN_DEFAULT(lv_style_t *style) {
    lv_style_set_min_width(style, 40);
    lv_style_set_max_width(style, 40);
    lv_style_set_min_height(style, 40);
    lv_style_set_max_height(style, 4);
...
void remove_style(lv_obj_t *obj, int32_t styleIndex) {
    ...
    static const RemoveStyleFunc remove_style_funcs[] = {
        ...
        remove_style_textarea_axis_value,
